...
class Ryanair_webdriver():
    ...
    def get_air_connections(self):
        ...
        for departure in air_connections.keys():
            selected_connections[departure] = list(set(air_connections[departure]) & set(destinations))
        final_connections = [[key, value] for key in air_connections.keys() for value in air_connections[key]]
    ...
    def get_flights(self, air_connections, main_currency):
        ...
        try:
            if len(flights_data["From"]) > 0:
                ...
                flights_df = flights_df.set_index(["Index", "Combined price", "Trip length", "From"]).sort_values(by=["Combined price", "Trip length", "Index"], ascending=[True, False, True])
            else:
